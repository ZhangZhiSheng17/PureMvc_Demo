...
public class CookMediator : Mediator
{
    ...
    {
        get { return (CookView) base.ViewCompoent; }
    }
    ...
    {
        base.OnRegister();
        cookProxy = Facades.RetieveProxy(CookProxy.NAME) as CookProxy;
        if (null == cookProxy)
        {
            throw new Exception(CookProxy.NAME + "is null.");
        }

        CookView.UpdataCook(cookProxy.Cooks);
    }
    ...
    public override void HandleNotification(INotification notification)
    {
        switch (notification.Name)
        {
            case OrderSystemEvent.CALL_COOK:
                ...
                SendNotification(OrderCommandEvent.CooKCooking,order);
                break;
